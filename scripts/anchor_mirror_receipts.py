#!/usr/bin/env python3
"""Seed demo receipts on a deployed Sentinel registry mirror."""
...
from deploy_registry import (  # noqa: E402
    CONTRACT_NAME,
    DEPLOYMENTS_FILE,
    compile_contract,
    load_private_key,
...
def main() -> int:
    ...
    parser.add_argument("--network", default="megaeth_testnet")
    parser.add_argument("--key-alias", help="Use a different configured testnet key.")
    ...
    if not network.deploy_enabled or not network.rpc or not network.chain_id:
    ...
    if not w3.is_connected():
        raise RuntimeError(f"RPC unreachable: {network.rpc}")
    if w3.eth.chain_id != network.chain_id:
        raise RuntimeError(f"Unexpected chain id: {w3.eth.chain_id}")

    account = Account.from_key(load_private_key(network.id, key_alias=args.key_alias))
    ...
    if _mandate_unknown(contract, mkey):
        ...
        demo_events["register_mandate"] = _tx_record(network, tx_hash)
    ...
    for label, decision, approved_flag in (
        ("approved_receipt", approved, True),
        ("blocked_receipt", blocked, False),
    ):
        if _receipt_unknown(contract, decision.receipt_id):
            tx_hash = _record(
                contract,
                w3,
                account,
                network.chain_id,
                {
                    ...
                    "mandate_key": mkey,
                    "payer": mandate.agent,
                    "amount_atomic": int(decision.payment_requirements.amount),
            ...
            demo_events[label] = _tx_record(network, tx_hash, receipt_id=decision.receipt_id)
            print(f"recorded {label}: {tx_hash}")
        else:
    ...
    print(f"remaining atomic spend: {demo_events['remaining_spend_atomic']}")
    return 0


def _send(w3: Any, account: Any, chain_id: int, fn: Any) -> str:
...
def _tx_record(network: Any, tx_hash: str, *, receipt_id: str | None = None) -> dict[str, str]:
    record = {
        "tx_hash": tx_hash,
        "explorer": f"{network.explorer}/tx/{tx_hash}" if network.explorer else tx_hash,
    }
    if receipt_id:
