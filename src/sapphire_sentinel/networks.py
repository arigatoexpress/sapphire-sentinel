...
NETWORKS: tuple[NetworkProfile, ...] = (
    ...
    NetworkProfile(
        ...
        status="fhevm_artifact_next",
        next_action="Ship EncryptedRiskGate with local mock tests, then deploy to Sepolia for real encryption.",
    ...
    NetworkProfile(
        ...
        rpc=None,
        explorer=None,
        status="blueprint_artifact_next",
        next_action="Add Noir private-intent circuit that exports a commitment to the public receipt path.",
...
INTEGRATION_PHASES: tuple[IntegrationPhase, ...] = (
    ...
    IntegrationPhase(
        ...
        status="next",
        proof="Privacy commitments already bind policy, resource, result, and risk hashes.",
        implementation="Add FHEVM local mock contract plus Sepolia deployment notes for encrypted budget/risk checks.",
        boundary="Do not claim native Robinhood Chain FHE; the real encrypted run is Zama-on-Sepolia.",
    ),
    IntegrationPhase(
        label="4. Aztec Intent Proof",
        status="blueprint",
        proof="Receipt model accepts an external privacy commitment without revealing prompt or portfolio data.",
        implementation="Add Noir scaffold proving a private mandate note exports the same public commitment.",
